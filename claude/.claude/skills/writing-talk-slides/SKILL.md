...
Translate an approved concept into `slides.md` — a per-slide spec with **Type / Visual / Layout / Speaker notes / Time**. Two principles run through every slide: **clear** (one idea, glance-readable) and **vivid** (concrete, named, sized). The skill is design-agnostic on visual choices but enforces two structural rules: a design language must exist before drafting, and every figure is generated from a committed source. This template already provides the figure pipeline — draw.io sources in `diagrams/` become full-bleed reveal frames via `diagrams/build_diagrams.py` — so reference it; don't invent a new one.
...
### Gate 2 — Committed-Source-Figures Gate

Every figure on every slide is generated from a source file committed in the talk repo. Defaults for this template:

| Figure type | Tool | Source | Output |
|---|---|---|---|
| Architecture / system / process / dataflow diagram | **draw.io** | `.drawio` in `diagrams/` | cumulative-reveal PNGs `figures/<stem>_NN.png` (via `./build.sh diagrams`) |
| Chart, plot, timeline, bar/line/scatter (rare) | **matplotlib** | `.py` in `tools/` | PNG/SVG in `figures/` |
| Geographic / map | draw.io or matplotlib | as above | as above |

Slide specs reference the **source** by path (`diagrams/<name>.drawio`), never the rendered PNG. **Forbidden:** hand-placed shapes in a slide tool, screenshots of dashboards or third-party diagrams, stock imagery, clip art, AI-generated decoration, pre-rendered images without a committed source.

**Multi-step diagrams are specified as a reveal sequence.** A diagram that builds up is one `.drawio` with one reveal layer per beat (see `diagrams/README.md` for the `lyr_*` layer convention). In the slide's `Visual:` block, list the beats — one line per layer, in reveal order — so the deck author knows how many frames to expect.

If the design doc names different tools, follow the design doc. Absent that: draw.io for diagrams, matplotlib for the occasional chart.
...
**Visual:**
- [What appears on the slide. Quoted text is exact wording. Reference figure source files by path: `diagrams/checkout-flow.drawio`. For a multi-step diagram, list the reveal beats — one line per layer, in order.]
...
3. **Figure audit** — every diagram has a `.drawio` source in `diagrams/`; multi-step diagrams list their reveal beats; any chart has a `.py` source in `tools/`. All referenced by source path, not by rendered PNG.
...
| Diagram described in prose only | Add the `diagrams/<name>.drawio` source path. Create the file if absent. For a build, list the reveal beats. |
| Chart inserted as a screenshot | Replace with a matplotlib script in `tools/`. |
...
| "I'll insert a screenshot of the diagram" | Build the `.drawio` source instead (reveal layers if it's multi-step). |
...
- **Figure index** — list every `diagrams/*.drawio` referenced by the deck, with its reveal-frame count (and any chart `.py` sources)

## Terminal State

Skill ends when `slides.md` is saved AND every referenced figure source (`.drawio` in `diagrams/`, plus any chart `.py`) exists on disk. **Do not** build the `.pptx`, render visuals, or rehearse delivery — those are separate steps. If the user asks for a deck build immediately after, treat it as a new request.
