...
function _worktree {
    ...
    # Local change: upstream writes `[ type -t direnv 2>/dev/null && -f ... ]`, which is not valid
    # test syntax — bash reports a missing `]`, then tries to run `-f` as a command.
    if command -v direnv >/dev/null 2>&1 && [ -f "$parent_dir/$dirname/.envrc" ]; then
